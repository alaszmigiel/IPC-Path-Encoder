...
//zwalnianie pamieci wspoldzielonej i zamykanie pliku
void release_resources(char* shm_ptr, FILE* file) 
{
    if (shm_ptr != NULL) 
	{
        if (shmdt(shm_ptr) == -1) 
...
void signal_handler_SIGUSR1(int signo)
{
    ...
    release_resources(shm_ptr, file);
    exit(0);
}
...
int main(int argc, char* argv[]) 
{
    ...
    //wypis sciezek zakodowanych 
    while (1) 
	{
        semaphore_wait(semid, 2);
        strcpy(hex, shm_ptr);

        if (strcmp(hex, "brak") == 0) 
		{
            break;
        }
